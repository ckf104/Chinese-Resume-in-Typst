...
- 在光轮智能实习四个月, 使用 UE5 生成合成数据和渲染三维重建的场景为自动驾驶算法训练提供支持
- *拓展 UE 的渲染管线*，在 UE 的渲染管线中添加自定义的 mesh pass processor 和 shader, 并在 RDG 中注册新的渲染 pass, 使 UE 额外输出一张应用自定义的 32bits 模板值的颜色纹理, 支持应用的语义分割需求
