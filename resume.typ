...
- *拓展 UE 的渲染管线*。在代码中读取配置文件, 在 primitive component 的 CustomPrimitiveData 中保存物体 ID。添加自定义的 mesh pass processor 和 shader, 并在 RDG 中注册新的渲染 pass, 使 UE 额外输出一张表示每个像素对应的物体 ID 的纹理, 并异步存盘。支持应用的语义分割需求
- 使用 *Niagara* 插件实现和优化 *Gaussian Splatting* 重建后的场景渲染。将 CUDA 实现的前向渲染迁移到 Niagara 的 shader 里。对于三百万个粒子点的场景, 使用 3 阶球谐系数描述椭球颜色时也能达到近 100 FPS 的渲染速度
- *拓展 UE 的资产管理模块*。编写点云文件的加载程序, 支持 0-3 阶的球谐系数, 使得在编辑器中可以将算法部门训练好的点云文件自动转化为纹理材质, 并且能够脚本化批量处理
- 通过 CUDA IPC 的方法将 UE 的渲染图像直接传输给智驾算法, 大大提升了以前基于落盘转存方法的效率
...
- 深刻理解基于蒙特卡洛采样的*路径追踪和体渲染算法*, 以及 *MIS*, *NEE* 等在工程实现上的许多优化技巧
...
#list(
[*Verilator 项目开源贡献*：为开源项目 Verilator（著名的 SystemVerilog 模拟器）贡献了约 1000 行代码，实现了 SystemVerilog 的 packed struct 到 C++ struct 的自动转换，解决了持续十年的功能需求#link("https://github.com/verilator/verilator/issues/860")[\#860]],
[*GitHub项目*#link("https://github.com/ckf104/user-level-tcp-ip")[user-level-tcp-ip]：使用C++和基于Linux的监听套接字实现用户态TCP/IP协议栈，提供POSIX套接字接口，支持与Linux/Windows主机直接通信],
[*本科毕业设计*：基于 LLVM 编译框架，将论文 _VeGen: a vectorizer generator for SIMD and beyond_ 中基于 SLP 和 pattern match 的向量化方法移植到 RISC-V 平台，项目被评为优秀本科毕业论文],
[*第一届全国大学生计算机系统能力大赛操作系统设计大赛二等奖*：改进 xv6-k210 操作系统，使其支持段式内存管理和惰性内存加载, 并能够在 K210 硬件平台上支持链接标准 C 库的应用程序运行],
[*阅读清华 Ventus-GPGPU 源码*：该 GPGPU 项目由 Chisel 编写，面向 OpenCL 程序，涵盖了 SIMT 栈、共享内存、屏障等基本功能。深入理解了 GPGPU 的工作原理及相关技术细节],
)
